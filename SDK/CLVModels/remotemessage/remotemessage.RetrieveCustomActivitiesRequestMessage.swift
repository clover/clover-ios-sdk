...
extension CLVModels.Remotemessage {
    ...
    public class RetrieveCustomActivitiesRequestMessage: Codable {
        ...
        required public init(from decoder: Decoder) throws {
            ...
            if (rootContainer.contains(.filters)) {
                do {
                    let filtersContainer = try rootContainer.nestedContainer(keyedBy: CLVModels.ElementsKey.self, forKey: .filters)
                    self.filters = try filtersContainer.decodeIfPresent([String].self, forKey: .elements)
                } catch {
                    decodeError.append(error)
                }
            }
